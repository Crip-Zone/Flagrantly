...
function template_body_above()
{
    ...
    echo ' 
					<div id="inner_wrap" class="floatright" ', !$context['user']['is_logged'] ? ' ' : '', '>';
...
// If the user is logged in, display some things that might be useful.
	...
	echo '
				</div>'; //end of inner wrap within header.
						...
						if ($context['allow_search'])
	{
		echo '
			<form id="search_form" class="floatright" action="', $scripturl, '?action=search2" method="post" accept-charset="', $context['character_set'], '">
				<input class="top_search" type="search" name="search" value="', $txt['search'], '" onfocus="this.value = \'\';" onblur="if(this.value==\'\') this.value=\'', $txt['search'], '\';" >&nbsp;';

		// Using the quick search dropdown?
		$selected = !empty($context['current_topic']) ? 'current_topic' : (!empty($context['current_board']) ? 'current_board' : 'all');

		echo '
				<select class="top_search" name="search_selection">
		...
		if (!empty($context['current_topic']))
			echo '
				<input class="top_search" type="hidden" name="sd_topic" value="', $context['current_topic'], '">';

		// If we're on a certain board, limit it to this board ;).
		elseif (!empty($context['current_board']))
			echo '
				<input class="top_search" type="hidden" name="sd_brd" value="', $context['current_board'], '">';

		echo '
				<input class="top_search button" type="submit" name="search2" value="', $txt['search'], '">
				<input class="top_search" type="hidden" name="advanced" value="0">
			</form>';
	}
